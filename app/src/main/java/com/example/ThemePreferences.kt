...
object ThemePreferences {
    ...
    private const val KEY_DISCORD_NOTIFIED = "discord_notified"

    const val DEFAULT_BUBBLE_SIZE = 56

    fun isDiscordNotified(context: Context): Boolean {
        val prefs = context.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
        return prefs.getBoolean(KEY_DISCORD_NOTIFIED, false)
    }

    fun setDiscordNotified(context: Context, notified: Boolean) {
        val prefs = context.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
        prefs.edit().putBoolean(KEY_DISCORD_NOTIFIED, notified).apply()
    }
